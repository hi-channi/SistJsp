...
<%
	...
	List<SmartboardDto> list=dao.getList(start, perPage);
...
<table class="table table-hover" style="width: 800px;">
	<caption>게시글 목록&nbsp;<span class="badge">총 <%=totalCount %>개</span>&nbsp;
		<button class="btn btn-success btn-xs glyphicon glyphicon-pencil" onclick="location.href='index.jsp?main=board/smartform.jsp'">작성하기</button>
	</caption>
	...
	<% } else {
		...
		<tr>
			...
			<td><a href="index.jsp?main=board/detailcontent.jsp?num=<%=dto.getNum() %>&currentPage=<%=currentPage %>"><%=dto.getSubject() %></a>
			</td>
			<td><%=dto.getWriter() %></td>
...
<div style="width: 500px; text-align: center;" class="container">
 <ul class="pagination">
         ...
         if(startPage>1){
         	%>
			<li>
				<a href="index.jsp?main=board/smartlist.jsp?currentPage=<%=startPage-1%>">이전</a>
			</li>
         <%}
         for(int pg=startPage;pg<=endPage;pg++) {
			...
			%>
			<li class="active">
				<a href="index.jsp?main=board/smartlist.jsp?currentPage=<%=pg %>"><%=pg %></a>
			</li>
			<% } else {
				%>
				<li>
		...
		if(endPage<totalPage){
			%>
			<li>
				<a href="index.jsp?main=board/smartlist.jsp?currentPage=<%=endPage+1%>">다음</a>
 			</li>
         <% }
         %>
  </ul>
